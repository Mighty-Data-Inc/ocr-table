...
describe('ocrTranscribeTableRowsFromCurrentPage (live API)', () => {
  ...
  it('transcribes only table rows on current page but recognizes clean-break continuation', async () => {
    ...
    // result.rows and wildernessProvisionsTable9Partial. We expect there to be at most
    // 1 differences.
    const numberOfDifferences = _countObjectDifferences(
      result.rows,
      wildernessProvisionsTable9Partial
    );

    expect(numberOfDifferences).toBeLessThanOrEqual(1);
